...
const CANONICAL_HOST = 'www.codebustersconnect.com';
const LEGACY_HOSTS = new Set(['codea-buster.vercel.app', 'cbdeploy.vercel.app']);

const SEO_AND_STATIC_PATHS = new Set([
  '/robots.txt',
  '/sitemap.xml',
  '/favicon.ico',
...
function isStaticAsset(pathname: string): boolean {
  ...
  return /\.(?:png|jpe?g|gif|svg|webp|ico|txt|xml|json|js|css|map|woff2?|ttf|otf)$/i.test(pathname);
}

export async function middleware(request: NextRequest) {
  const { pathname, hostname, search } = request.nextUrl;

  // Redirect legacy/public Vercel domains to canonical domain
  ...
  if (!isAdmin && !isMaintenancePage && !isApi && !isSeoOrStatic) {
  ...
  if (!isPublicRoute && !isPublicPathPrefix && !isAdmin && !isSeoOrStatic) {
...
export const config = {
  matcher: ['/((?!_next/static|_next/image|Logo_CBP.png).*)'],
};
